        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-12">
            <div class="flex justify-between h-20 items-center">
                ...
                <div class="hidden md:flex space-x-8 items-center">
                    ...
                    <a href="/chaussures" class="text-white text-sm font-medium relative after:content-[''] after:absolute after:-bottom-2 after:left-0 after:w-full after:h-0.5 after:bg-accent-500">Chaussures</a>
    ...
    <main class="flex-grow pt-32 pb-20 px-6 lg:px-12 relative z-10 border-t-0">
        <div class="max-w-7xl mx-auto">
            <header class="mb-16 text-center md:text-left">
                ...
                <p class="text-slate-400 font-light text-lg max-w-2xl mx-auto md:mx-0">Parcourez notre collection de chaussures de basketball haute performance et trouvez la paire idéale pour votre prochaine session.</p>
            </header>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                @foreach ($chaussures as $chauss)
                    <div class="glass-card rounded-[1.5rem] p-6 group relative overflow-hidden flex flex-col justify-between h-full bg-dark-800/80">
                        <div class="absolute inset-0 bg-gradient-to-br from-indigo-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                        </div>

                        <div class="relative z-10 flex-grow">
                            <!-- Graphical representation of Shoe -->
                            <div class="h-48 mb-6 flex items-center justify-center relative">
                                @if($chauss->image)
                                    <div class="w-full h-full rounded-2xl overflow-hidden shadow-xl group-hover:scale-105 transition-transform flex items-center justify-center border border-white/5">
                                        <img src="{{ asset('storage/' . $chauss->image) }}" class="w-full h-full object-cover" alt="{{ $chauss->modele }}">
                                    </div>
                                @else
                                    <div class="w-24 h-24 rounded-2xl bg-gradient-to-br from-indigo-500/20 to-purple-500/20 border border-indigo-500/30 shadow-xl group-hover:scale-105 transition-transform flex items-center justify-center rotate-[-15deg]">
                            ...
                            <div class="flex items-center gap-2 mb-2">
                                <span class="px-2 py-0.5 rounded text-[10px] uppercase tracking-wider font-bold bg-indigo-500/20 text-indigo-300">{{ $chauss->marque }}</span>
                            </div>
                            <h3 class="text-xl font-serif text-white mb-1 group-hover:text-indigo-300 transition-colors line-clamp-1 truncate uppercase tracking-widest">{{ $chauss->modele }}</h3>
                            
                            <div class="flex justify-between items-center mb-6">
                                <p class="text-slate-400 text-sm font-bold">Pointure: <span class="text-white">{{ $chauss->pointure }}</span></p>
                                <span class="px-2 py-0.5 rounded text-[10px] uppercase tracking-wider font-bold bg-white/10 text-slate-300">{{ $chauss->etat }}</span>
                            </div>
                        </div>

                        <!-- Emprunter Action -->
                        <form action="{{ route('chaussures.emprunter', $chauss->id_chaussure) }}" method="POST" class="relative z-10 mt-auto">
                            @csrf
                            <button type="submit"
                                class="w-full py-3 rounded-xl btn-outline flex items-center justify-center gap-2 text-white font-medium hover:border-indigo-400/50 hover:bg-indigo-400/10 transition-all cursor-pointer">
